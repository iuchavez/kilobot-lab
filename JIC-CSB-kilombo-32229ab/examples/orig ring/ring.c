...
void recv_election(uint8_t *payload)
{
    ...
    mydata->blue = 0;
    mydata->green = 0;
    if (payload[MINID] < mydata->min_id)
    {
        mydata->min_id = payload[MINID];
        mydata->ready_to_election = TRUE;
    }
    else if (payload[MINID] > mydata->min_id && !mydata->has_particapted)
    {
        mydata->ready_to_election = TRUE;
    }
    else{
            // A leader has been elected
        mydata->ready_to_elected = TRUE;
    }

    mydata->has_particapted = TRUE;
}


void recv_elected(uint8_t *payload)
{
    mydata->has_particapted = FALSE;
    if (payload[MINID] != mydata->my_id){
        mydata->ready_to_elected = TRUE;
        mydata->blue = 255;
        mydata->red = 0;
        mydata->green = 0;
...
char enqueue_message(uint8_t m)
{
 ...
 //   printf("%d, Prepare %d\n", mydata->my_id, m);
	...
	//	mydata->message[mydata->tail].data[COLOR] = RGB(mydata->red,mydata->green,mydata->blue);
        ...
        if (m == LEADER_ELECTION || m == LEADER_ELECTED)
...
void  send_elected()
{
    if (mydata->ready_to_elected && !isQueueFull())
    {
        enqueue_message(LEADER_ELECTED);
        mydata->ready_to_elected = FALSE;
    }
...
void  send_election()
{
    if (mydata->ready_to_election && !isQueueFull())
    {
        enqueue_message(LEADER_ELECTION);
        mydata->ready_to_election = FALSE;
        mydata->red = 255;
         mydata->blue = 0;
        mydata->green = 0;
    }
